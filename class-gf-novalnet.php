...
class GF_Novalnet extends GFPaymentAddOn {
	...
	public function handle_payment_status( $request, $entry, $settings, &$action ) {
		...
		if ( in_array( rgar( $request, 'tid_status' ), $this->_payment_complete_status, true ) ) {
			if($request['key'] == '41'){
				$action['payment_status'] = $settings['payment_callback_status'];
			}else{
	...
	public function redirect_url( $feed, $submission_data, $form, $entry ) {
		...
		GF_Novalnet_Helper::form_motoForm_parameters( $parameters );
	...
	public function feed_settings_fields() {
		...
		array_unshift( $billing_info['field_map'], array( 'name' => 'birth_date', 'label' => esc_html__( 'Birth Date', 'gravityforms-novalnet' ), 'required' => false ) );
		
		array_unshift( $billing_info['field_map'], array( 'name' => 'company', 'label' => esc_html__( 'Company Name', 'gravityforms-novalnet' ), 'required' => false ) );
		
		array_unshift( $billing_info['field_map'], array( 'name' => 'phone', 'label' => esc_html__( 'Telephone Number', 'gravityforms-novalnet' ), 'required' => false ) );
		
		array_unshift( $billing_info['field_map'], array( 'name' => 'mobile', 'label' => esc_html__( 'Mobile Number', 'gravityforms-novalnet' ), 'required' => false ) );
