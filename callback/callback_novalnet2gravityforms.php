...
class Novalnet_vendor_script {
    ...
    protected $ary_payments = array( 'INVOICE_START', 'PRZELEWY24', 'CASHPAYMENT','PAYPAL','ONLINE_TRANSFER','CREDITCARD','GIROPAY', 'IDEAL', 'EPS', 'DIRECT_DEBIT_SEPA' );
    // @Array Type of Chargebacks available - Level : 1
    protected $ary_chargebacks = array( 'RETURN_DEBIT_SEPA', 'PRZELEWY24_REFUND', 'REFUND_BY_BANK_TRANSFER_EU', 'REVERSAL', 'CASHPAYMENT_REFUND', 'CREDITCARD_BOOKBACK','CREDITCARD_CHARGEBACK', 'PAYPAL_BOOKBACK' );
    protected $ary_subscription = array( 'SUBSCRIPTION_STOP' );
    // @Array Type of CreditEntry payment and Collections available - Level : 2
    protected $ary_collection = array( 'INVOICE_CREDIT', 'CREDIT_ENTRY_CREDITCARD', 'CREDIT_ENTRY_SEPA', 'DEBT_COLLECTION_SEPA', 'DEBT_COLLECTION_CREDITCARD', 'CASHPAYMENT_CREDIT', 'ONLINE_TRANSFER_CREDIT');
    protected $ary_payment_groups = array(
                                'novalnet_creditcard'         => array( 'CREDITCARD', 'DEBT_COLLECTION_CREDITCARD', 'CREDIT_ENTRY_CREDITCARD', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'SUBSCRIPTION_STOP' ),
                                'novalnet_sepa'               => array( 'DIRECT_DEBIT_SEPA', 'DEBT_COLLECTION_SEPA', 'RETURN_DEBIT_SEPA', 'CREDIT_ENTRY_SEPA', 'SUBSCRIPTION_STOP' ),
                                'novalnet_ideal'              => array( 'IDEAL', 'REFUND_BY_BANK_TRANSFER_EU', 'ONLINE_TRANSFER_CREDIT', 'REVERSAL'),
                                'novalnet_online_transfer'    => array( 'ONLINE_TRANSFER', 'ONLINE_TRANSFER_CREDIT', 'REFUND_BY_BANK_TRANSFER_EU', 'REVERSAL' ),
                                ...
                                'novalnet_przelewy24'         => array( 'PRZELEWY24', 'PRZELEWY24_REFUND', 'REFUND_BY_BANK_TRANSFER_EU' ),
                                ...
                                'novalnet_invoice_start'      => array( 'INVOICE_START',  'INVOICE_CREDIT', 'SUBSCRIPTION_STOP' ),
    ...
    */
    function validate_capture_params( $ary_capture ) {
        ...
        if ( !isset( $ary_capture['vendor_activation'] ) ) {
            ...
            else if ( in_array( $ary_capture['payment_type'], array_merge( $this->ary_chargebacks, $this->ary_collection ) ) ) {
    ...
    */
    function get_order_reference() {
        ...
        if ( $order_id ) {
            $novalnet_response['amount'] = $wpdb->get_var( $wpdb->prepare( "SELECT payment_amount FROM ".GFFormsModel::get_lead_table_name(). " WHERE id = %s", $order_id ) );
            $dbValue['tid']                    = $tid;
  ...
  */
    ...
    */
    function send_notify_mail( $datas = array(), $ary_captureParams ) {
        $email_from         = ''; //sender email address, mandatory, adapt it
        $email_from_name    = ''; //sender name, adapt 
        $email_to           = ''; //recipient email address, mandatory, adapt it
        $email_to_name      = ''; //recipient name, adapt
    ...
    */
    function validateIpAddress() {
        ...
        {
            echo "Novalnet HOST IP missing";
        }        
        if ( $get_host_name != $client_ip && ! $this->process_test_mode )
        {
